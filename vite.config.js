...
/**
 * The card, built second - `npm run build` runs the editor's build first.
 *
 * A function rather than an object because of the file read below: vitest
 * loads this config too, to serve the test run, and it has no dist/ to read.
 * Everything here is for `vite build` and nothing else asks for it.
 */
export default defineConfig(({ command }) => {
  if (command !== 'build') return {};

  // Written by the editor build. Its name carries a content hash, and
  // `getConfigElement` imports exactly that file.
  let editorChunk;
  try {
  ...
  return {
    ...
    build: {
      ...
      outDir: 'dist',
      // The editor build ran first and cleared the directory; clearing it
      // again would delete the file this one imports.
      emptyOutDir: false,
      rollupOptions: {
        // The editor is a build of its own, so there is nothing here to
        // resolve: the name is a file beside this one at runtime, not a
        // module to pull in.
